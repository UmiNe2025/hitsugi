...
## ③-2 第2局面 完了済み(証跡)
- **Codex基盤** ✅ `d412f23`(2093行・build/lint/vitest/validate緑を統合者確認。精読レビューは未実施)。
- **M5 敵の兆し** ✅ `280ab96`(enemyAction不変=rngクローン先読み・攻/術/群3種・vitest 528ゴールデン+playwrightバッジ)。
- **M26 P0 星契り/郷普請 確認Sheet** ✅ `c25675c`(誤操作即確定を廃止・playwright実測で消費前不変)。
- **M26 P0 clickable div→button** ✅ `2b5c807`(家系図ノード・ホーム日参り)。
- **M7 図鑑個別既読** ✅ `4a40e89`(件数マーク→ID集合・忠実移行・vitest 533+playwright mount非既読化)。
- **M6 room archetype**:
  - **安全中核(石碑≥1/地域・特殊床到達可能性)= Codex基盤 `tests/generated_maps.test.ts` が既に実装+緑**(line 73/76/79。devil指摘の恒久バグ「収集到達不能」を機械保証)。**私が全内容を検証**。
  - Codexは27→39地域(171層・全ユニーク・地域固有)へ拡張済み。
  - **6種の名前付きarchetype(門前/広間/回廊/中庭/橋狭路/段丘)= 品質機能は未実施(⚠️)**。理由: 全171層の再生成はCodexの地域固有マップ(ユーザー報告バグを直した成果)を上書きする高リスク作業で、専用セッションで安全に行うべき。安全中核は満たされ、ユーザーの実害(灯篭崩れ道)はCodexマップで解消済み。
- **M26 P0 場面送り誤タップ防止** ✅ `e160114`(委譲停滞→本体で実装。背景クリックのみ進行+250msデバウンス+Space/Enter進行。playwright実測)。
- **灯篭崩れ道バグ**: 現ツリーで正常描画(10通り検証)。デプロイ版(旧M24)の陳腐化。**push で解消**(ユーザーゲート・未実施)。

### 第2局面 監査表(自己監査)
| 項目 | 判定 | 証跡 |
|---|---|---|
| Codex基盤統合 | ✅(精読⚠️) | `d412f23`。build/lint/vitest/validate緑。実装ロジックの精読レビューは未実施(ユーザー合意:機械検証のみ) |
| M5 敵の兆し(AI不変) | ✅ | `280ab96`。vitest 533(computeIntents実rng不消費・enemyAction 1000シード決定論)+playwrightバッジ |
| M6 安全中核(石碑≥1/到達性) | ✅ | Codex `generated_maps.test.ts`(line73/76/79)を検証。171層ユニーク・到達可能 |
| M6 6種archetype品質 | ❌(スコープ判断) | 全171層再生成はCodex地域固有マップを上書きする高リスク。専用セッション推奨。安全中核は満たされユーザー実害は解消済み |
| M7 図鑑個別既読 | ✅ | `4a40e89`。vitest 533(忠実移行/冪等/変異種/後方互換)+playwright mount非既読化 |
| M26 P0 確認Sheet×3 | ✅ | 血潮鍛錬`22dfad2`/星契り・郷普請`c25675c`。playwright消費前不変 |
| M26 P0 button化 | ✅ | `2b5c807`(家系図・日参り) |
| M26 P0 場面送り | ✅ | `e160114`。playwright本文非進行 |
| 統合 | ✅ | build/lint/vitest533/validate/index.css非編集/playwright統合34+個別緑 |
| 灯篭崩れ道 | ✅(要deploy) | 現ツリー10通り正常描画。デプロイ版の陳腐化→push で解消(ユーザーゲート) |

## ⑨terminal印(第2局面)
**部分達成(terminal)** — 2026-07-16 JST。M5・M7・M26 Phase0全項目・M6安全中核=✅(機械検証+実測)。**M6の6種archetype品質のみ❌(スコープ判断:Codexマップ上書きの高リスクを回避)**。Codex基盤は精読未レビュー⚠️。灯篭崩れ道はデプロイで解消(push未実施・ユーザーゲート)。第1局面(M25 Phase1-2+M8)も達成済み。
...
## 第3局面(2026-07-17): 公開後のユーザー実走で発覚した3件
| 項目 | 判定 | 証跡 |
|---|---|---|
| マップ上の金色人型(根絶) | ✅ | `9769eff`。根因=walke_/walkc_全144枚がM23工場汚染(人物→金紋様)。elder当主で発症。退避+郷二段フォールバック+回帰テスト。elder実測で武者描画 |
| 戦闘オート/技が押せない | ✅ | `4255b36`。根因=.stage-ground(装飾)がpointer-events無しでコマンド盤を覆いクリック横取り。実測: オート⇔オート中切替・技メニュー展開 |
| 戦闘デザイン(ultrawide散開) | ✅ | 同上。戦場max-width 1480+前後列の縦分裂是正+オートのボタン化。戦闘gate 25/25全viewport緑 |
- 保留: walke_/walkc_ 144枚の正規再生成(ComfyUI工場・修正済みプロンプトで。復帰手順はquarantine/README)。
- terminal(第3局面): **達成**。push待ち(ユーザーゲート)。
