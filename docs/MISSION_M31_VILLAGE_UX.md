...
| G 統合 | ✅ | build緑/lint0/vitest**552**/**playwright 190 passed**/index.css非編集/**push無し**。 |

## ⑨terminal印
**達成**(2026-07-17)。A/B/C/D の4要望を全て適用・実証(A=常設オート切替・B=事件の目的明示・
C=見世崩れ是正[M28-C回帰]・D=郷3画面のデザイン向上)。加えてD報告の既存mobile横スクロールも是正。
全て build緑/lint0/vitest552/playwright190で実証・commit済み・**push無し**(公開はユーザー判断)。
Dの「魅力」の主観部のみ⚠️(ユーザー実走で最終確認)。

**ユーザーゲート(繰延)**: push=公開デプロイ。本ミッションの全commitがローカル待機。
